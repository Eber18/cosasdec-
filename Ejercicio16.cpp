...
class Nodo {
      public: T info;
              int num,distancia;
              Nodo<T> *sig;
              Nodo(int num,string T,int distancia);
};
...
class Cola {
      ...
      public:
             Cola();
             ~Cola();
             void Put(int num,string T,int d);
             T Get();
             void Ver();
             int Vacia();
};
...
template<class T>
Nodo<T>::Nodo(int n,string s,int d) {
                  num=n;
                  info = s;
                  distancia=d;
                  sig = NULL;
...
template<class T>
void Cola<T>::Put(int n,string s,int d) {
     Nodo <T>*nvo= new Nodo<T>(n,v,d);
     if(Vacia())
                Ini=Fin=nvo;
     else {
          Fin->sig=nvo;
